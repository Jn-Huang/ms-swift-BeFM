...
#SBATCH --job-name=swift_llama3_70b_lora
#SBATCH --output=/home/huangjin/ms-swift-BeFM/logs/llama3_70b_lora_%j.log
...
uv run python -c "import deepspeed; print(deepspeed.__version__)"
echo "=========================================="

# Run training with ms-swift
# LLaMA 3.3-70B with LoRA + DeepSpeed ZeRO-2 on 4x A100 GPUs
echo "Starting LLaMA 3.3-70B LoRA SFT training with 4 GPUs + DeepSpeed ZeRO-2..."
...
uv run swift sft \
    ...
    --logging_steps 10 \
    --max_length 2048 \
    --output_dir /scratch/qmei_root/qmei/huangjin/models/ms-swift/llama3-70b-lora \
    --warmup_ratio 0.1 \
    --dataloader_num_workers 4 \
    --deepspeed zero3 \
    --report_to wandb \
    --run_name llama3_70b_lora_sft
